...
export default function Player() {
  // ✨ [수정 1] 주소창에서 epId뿐만 아니라 courseId(코너명)도 가져옵니다!
  const { courseId, epId } = useParams();
  ...
  // ✨ [수정 2] 코너명이 주소창에 없으면 기본값으로 'real-reaction' 지정
  const activeCourseId = courseId || 'real-reaction';

  // ✨ [수정 3] 버니넷 주소도 코너명에 따라 자동으로 바뀌도록 세팅!
  const CDN_BASE_URL = `https://talkori.b-cdn.net/podcast/${activeCourseId}`;
  ...
  const toggleVocaBookmark = (voca, e) => {
    ...
    if (isSaved) {
      ...
      setSavedVoca([...savedVoca, { ...voca, epId, courseId: activeCourseId, dateSaved: getTodayDate() }]); 
    }
  };
  ...
  const toggleScriptBookmark = (item, e) => {
    ...
    if (isSaved) {
      ...
      setSavedScript([...savedScript, { ...item, epId, courseId: activeCourseId, dateSaved: getTodayDate() }]); 
    }
  };
  ...
  const saveRadioTime = (e) => localStorage.setItem(`talkori_radio_pos_${activeCourseId}_${epId}`, e.target.currentTime);
  const loadRadioTime = (e) => {
    const savedTime = localStorage.getItem(`talkori_radio_pos_${activeCourseId}_${epId}`);
  ...
  const getEmbedUrl = (url) => {
    ...
    else if (url.includes('watch?v=')) embedUrl = url.replace('watch?v=', 'www.youtube.com/embed/');
  ...
  useEffect(() => {
    ...
    // ✨ [수정 4] 이제 헷갈리지 않고 정확한 폴더의 JSON을 가져옵니다!
    import(`../data/${activeCourseId}/ep${epId}.json`)
      .then((module) => {
  ...
  const playLine = (index) => {
    ...
    if (!item || !item.audio) {
      if (autoPlayRef.current && index < epData.content.length - 1) playLine(index + 1);
      else { setIsPlayingAll(false); setCurrentIndex(null); }
      return;
    }

    // ✨ [오디오 경로] CDN_BASE_URL 이 이미 코너명에 맞게 세팅되어 있음!
    ...
    if ('mediaSession' in navigator) {
      ...
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        album: currentCourse?.title[lang] || 'Talkori Podcast',
      });
    }

    audio.onended = () => {
      if (autoPlayRef.current) playLine(index + 1);
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-screen w-full bg-gray-50"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-indigo-600"></div></div>;
  ...
  if (!epData) return (
    <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gray-50">
      ...
      <Link to={`/course/${activeCourseId}`} className="text-indigo-600 hover:underline">Back to list</Link>
    </div>
  );
      ...
      <div className="p-5 border-b border-gray-100 shrink-0 flex justify-between items-start">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-lg font-extrabold text-gray-900 leading-tight">{currentCourse?.title[lang] || 'Talkori Course'}</h2>
          </div>
          ...
          return (
            <div 
              ...
              onClick={() => { 
                ...
                // ✨ [수정 5] 리스트 클릭 시 코너명까지 주소에 포함시켜서 이동!
                if (!isCurrent) navigate(`/player/${activeCourseId}/${ep.id}`); 
              }} 
    ...
    <div className="flex w-full min-h-screen bg-gray-50 relative">
      ...
      {isPlaylistOpen && (
        <div className="lg:hidden fixed inset-0 z-[100] flex">
          ...
          <div className="relative w-4/5 max-w-sm bg-white h-full ml-auto shadow-2xl flex flex-col animate-slide-in-right">
      ...
      <main className="flex-1 w-full relative pb-32">
        <div className="max-w-4xl mx-auto md:px-4">
            ...
            <div className="px-3 pt-3 md:p-6 max-w-4xl mx-auto flex flex-col">
              ...
              <div className="flex items-center justify-between w-full mb-2 md:mb-4">
                <div className="flex-1 flex flex-col items-center mx-2 overflow-hidden">
                  ...
                  <h1 className="text-sm md:text-xl font-extrabold text-gray-900 truncate md:line-clamp-2 md:whitespace-normal w-full text-center">{displayTitle}</h1>
                </div>
                <button onClick={() => setIsPlaylistOpen(true)} className="text-indigo-600 bg-indigo-50 p-1.5 px-2.5 rounded-md text-xs font-bold shrink-0 md:hidden">☰</button>
                <div className="hidden md:block"><button onClick={cyclePlaybackRate} className="text-gray-500 bg-gray-100 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-200">Speed: {playbackRate}x</button></div>
              </div>

              <div className="flex items-center justify-between px-1 md:justify-center md:gap-10 mb-3 md:mb-5">
                <button onClick={cyclePlaybackRate} className="md:hidden text-[11px] font-bold text-gray-600 bg-gray-100 px-2 py-1.5 rounded-md w-12 text-center shadow-sm">{playbackRate}x</button>
                <div className="flex items-center gap-6 md:gap-10">
                  <button onClick={() => toggleMedia('video')} disabled={!epData.metadata.youtube} className={`flex flex-col items-center gap-1 transition-all ${!epData.metadata.youtube ? 'text-gray-200 opacity-50 cursor-not-allowed' : activeMedia === 'video' ? 'text-red-600 scale-110' : 'text-red-400 hover:text-red-500'}`}><span className="text-2xl md:text-3xl">📺</span><span className="hidden md:block text-[11px] font-bold">Video</span></button>
                  <button onClick={togglePlayAll} className={`${isPlayingAll ? 'bg-indigo-600' : 'bg-gray-900'} text-white w-12 h-12 md:w-16 md:h-16 rounded-full flex items-center justify-center shadow-md hover:bg-indigo-500 transition-transform`}><span className="text-lg md:text-2xl">{isPlayingAll ? '⏸' : '▶'}</span></button>
                  <button onClick={() => toggleMedia('radio')} disabled={!epData.metadata.full_audio} className={`flex flex-col items-center gap-1 transition-all ${!epData.metadata.full_audio ? 'text-gray-200 opacity-50 cursor-not-allowed' : activeMedia === 'radio' ? 'text-indigo-600 scale-110' : 'text-indigo-400 hover:text-indigo-500'}`}><span className="text-2xl md:text-3xl">🎧</span><span className="hidden md:block text-[11px] font-bold">Radio</span></button>
                </div>
                ...
                <div className="w-full mb-4 animate-fade-in px-1 md:px-8">
                  ...
                  {activeMedia === 'radio' && epData?.metadata?.full_audio && (
                    <div className="w-full bg-gray-50 p-2 md:p-3 rounded-xl border border-gray-200 shadow-inner flex flex-col items-center">
                      ...
                      <audio controls autoPlay controlsList="nodownload" onTimeUpdate={saveRadioTime} onLoadedMetadata={loadRadioTime} className="w-full h-10 md:h-12 outline-none"><source src={epData.metadata.full_audio} type="audio/mpeg" /></audio>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="flex border-t border-gray-100 pt-2 md:pt-4 w-full">
                <button onClick={() => handleTabChange('script')} className={`flex-1 pb-2 md:pb-3 text-sm md:text-base font-bold transition-colors ${activeTab === 'script' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>📝 대본 (Script)</button>
                <button onClick={() => handleTabChange('voca')} className={`flex-1 pb-2 md:pb-3 text-sm md:text-base font-bold transition-colors ${activeTab === 'voca' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-400 hover:text-gray-600'}`}>📚 단어장 (Vocabulary)</button>
              </div>
          ...
          <div className="bg-white md:rounded-3xl shadow-sm border-y md:border border-gray-200 p-4 md:p-8 mt-4">
            <h2 className="hidden md:block text-sm font-bold text-gray-400 uppercase tracking-widest mb-6 border-b border-gray-100 pb-2">{activeTab === 'script' ? 'Interactive Script' : 'Vocabulary List'}</h2>
                  ...
                  return (
                    <div key={index} ref={(el) => (lineRefs.current[index] = el)} className={`relative flex flex-col items-start gap-1 p-2 -mx-2 rounded-xl transition-all duration-300 ${isPlaying ? 'bg-indigo-50/50 border-l-4 border-indigo-400 pl-4' : 'border-l-4 border-transparent pl-4 hover:bg-gray-50/50'}`}>
                      <div className="flex justify-between items-center w-full">
                        <span className={`text-xs font-bold uppercase ${isMina ? 'text-indigo-500' : 'text-gray-400'}`}>{cleanSpeakerName}</span>
                        {item.playable && (
                          <button onClick={(e) => toggleScriptBookmark(item, e)} className={`text-xl transition-transform hover:scale-110 ${isSaved ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-400'}`}>{isSaved ? '★' : '☆'}</button>
                        )}
                      </div>
                      {item.playable ? (
                        ...
                        <p onClick={() => handleLineClick(index)} className={`text-lg px-2 py-1 -ml-2 rounded-lg transition-colors cursor-pointer pr-8 whitespace-pre-wrap ${isPlaying ? 'text-indigo-600 font-bold' : 'text-gray-800 hover:text-gray-600'}`}>{item.text}</p>
                      )}
                      ...
                      <div key={idx} className={`flex flex-col items-start gap-1 p-4 rounded-xl transition-all duration-300 border ${isPlayingVoca ? 'bg-indigo-50/50 border-indigo-200 shadow-sm' : 'bg-white border-gray-100 hover:border-indigo-100'}`}>
                        <div className="flex justify-between items-start w-full mb-1">
                          <h3 className={`text-xl font-extrabold flex items-center gap-2 ${isPlayingVoca ? 'text-indigo-700' : 'text-gray-900'}`}>
                            {voca.word}
                            <button onClick={(e) => toggleVocaBookmark(voca, e)} className={`text-xl transition-transform hover:scale-110 mb-1 ${isSaved ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-400'}`}>{isSaved ? '★' : '☆'}</button>
                          </h3>
                          <button onClick={() => playVoca(idx)} className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isPlayingVoca ? 'bg-indigo-600 text-white' : 'bg-gray-50 text-gray-400 hover:bg-indigo-50'}`}><span className="text-sm">{isPlayingVoca ? '🔊' : '▶'}</span></button>
                        </div>
            ...
            <div className="mt-16 pt-6 border-t border-gray-100 flex items-center justify-between gap-3 md:gap-4">
              {prevEp ? (
                <Link to={`/player/${activeCourseId}/${prevEp.id}`} className="flex-1 py-4 bg-gray-50 hover:bg-indigo-50 text-gray-700 hover:text-indigo-600 rounded-2xl flex items-center justify-center font-extrabold text-sm transition-colors shadow-sm">← Prev</Link>
              ) : (
                <div className="flex-1 opacity-50 cursor-not-allowed py-4 bg-gray-50 text-gray-400 rounded-2xl flex items-center justify-center font-extrabold text-sm">← Prev</div>
              )}

              <button onClick={() => setIsPlaylistOpen(true)} className="lg:hidden flex-1 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl flex items-center justify-center font-extrabold text-sm transition-colors shadow-md gap-2">☰ {lang === 'ko' ? '목록보기' : 'Playlist'}</button>

              {nextEp ? (
                <button
                  onClick={(e) => {
                    const nextIndex = playlistEps.findIndex(ep => ep.id === nextEp.id);
                    if (isDemoMode && nextIndex >= 3) { e.preventDefault(); setShowPremiumModal(true); } 
                    else { navigate(`/player/${activeCourseId}/${nextEp.id}`); } // ✨ [수정 6] 넥스트 버튼에도 코너명 포함
                  }}
                ...
                <div className="flex-1 opacity-50 cursor-not-allowed py-4 bg-gray-50 text-gray-400 rounded-2xl flex items-center justify-center font-extrabold text-sm">Next →</div>
              )}
            </div>
      ...
      {showPremiumModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[24px] p-8 md:p-10 max-w-sm w-full text-center shadow-2xl">
            <div className="text-5xl mb-4">🔒</div>
            <h3 className="text-2xl font-extrabold text-gray-900 mb-3 tracking-tight">Premium Content</h3>
            <p className="text-gray-500 text-sm leading-relaxed mb-8">This lesson is for premium members only.<br/>Upgrade now to get <strong className="text-gray-700">unlimited access to</strong><br/>all classes and the wordbook!</p>
            <button onClick={() => window.parent.location.href = 'https://talkori.com/price'} className="w-full bg-[#3b32f5] hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl transition-colors mb-4 shadow-md text-[15px]">Upgrade Now 🚀</button>
            <button onClick={() => setShowPremiumModal(false)} className="text-gray-400 text-sm font-medium hover:text-gray-600 transition-colors">Maybe later</button>
          </div>
        </div>
      )}
    </div>
